...
%% @spec inject(Node::atom(), Mod::atom(), false | {ssh, Path::string(), Port::integer()}) -> ok | error | {error, Reason::term()}
...
inject(Node, Mod, SSH) ->
    case loaded(Node) of
        false -> inject(Node, Mod, SSH, mods());
...
inject(Node, Mod, SSH, [M | Tail]) when is_atom(Mod) ->
    ...
    case rpc(Node, code, load_binary, [M, FName, Bin]) of
        {module, M} -> inject(Node, Mod, SSH, Tail);
